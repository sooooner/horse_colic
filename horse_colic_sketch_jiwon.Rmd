...
# value 담을 list
horse.value.list <- vector("list", length = 12)
horse.value2.list <- vector("list", length = 12)
...
for(i in 1:12){
horse.value2.list[[i]] <- categorical.horse[,c(13,i)]
}

names(horse.value.list) <- name.of.categorical.horse[1:12]
names(horse.value2.list) <- name.of.categorical.horse[1:12]
